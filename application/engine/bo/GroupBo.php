...
//		echo showQuery($query, $group);
	...
	function getMyGroups($filters) {
		...
		$query = "	SELECT dlp_groups.*, dlp_themes.*,
					...
					LEFT JOIN ".$this->database."galette_groups ggg ON ggg.id_group = ".$filterField."id AND ".$filterField."type = 'galette_groups'
					LEFT JOIN ".$this->database."galette_groups_members ggm ON ggg.id_group = ggm.id_group
					LEFT JOIN ".$this->database."galette_adherents gga ON gga.id_adh = ggm.id_adh

					-- The source of voting is maybe a galette adh
					LEFT JOIN ".$this->database."galette_adherents ga ON ".$filterField."type = 'galette_adherents'

					-- The source of voting is maybe a personae group
					LEFT JOIN dlp_groups dg ON dg.gro_id = ".$filterField."id AND ".$filterField."type = 'dlp_groups'
					LEFT JOIN dlp_group_themes dtg ON dg.gro_id = dtg.gth_group_id
					LEFT JOIN dlp_themes dt ON dt.the_id = dtg.gth_theme_id
					LEFT JOIN dlp_fixations df ON df.fix_id = dt.the_current_fixation_id
		...
		if (true) {
			...
			$query .= "
					LEFT JOIN ".$this->database."galette_cotisations _gc ON ga.id_adh = _gc.id_adh AND _gc.info_cotis = '' AND _gc.date_fin_cotis > NOW()
					LEFT JOIN ".$this->database."galette_types_cotisation gtc ON _gc.id_type_cotis = gtc.id_type_cotis AND gtc.cotis_extension = 1

					LEFT JOIN ".$this->database."galette_cotisations _dgc ON dga.id_adh = _dgc.id_adh AND _dgc.info_cotis = '' AND _dgc.date_fin_cotis > NOW()
		...
		if (isset($filters["the_id"])) {
			...
			$query .= "	AND dlp_themes.the_id = :the_id ";
		}

		if (!isset($filters["with_deleted"])) {
			$query .= " AND dlp_themes.the_deleted = 0 \n";
		}
		...
		$query .= "	ORDER BY dlp_groups.gro_label, dlp_themes.the_label ";
...
//		echo showQuery($query, $args);
	...
	function getGroups($filters = null) {
		...
		$query = "	SELECT *
					...
					RIGHT JOIN dlp_group_themes ON gth_group_id = gro_id
					RIGHT JOIN dlp_themes ON gth_theme_id = the_id AND gth_theme_type = 'dlp_themes'
					LEFT JOIN dlp_fixations ON fix_id = the_current_fixation_id
					LEFT JOIN dlp_fixation_members ON fix_id = fme_fixation_id
					LEFT JOIN ".$this->database."galette_adherents ON fme_member_id = id_adh
					WHERE
						1 = 1 \n";
...
//		echo showQuery($query, $args);
	...
	function addTheme($group, $theme, $power = 1) {
		...
		$query = "	INSERT INTO dlp_group_themes
	...
	function excludeTheme($group, $theme) {
		...
		$query = "	DELETE FROM dlp_group_themes
					WHERE gth_group_id = :gth_group_id \n";
	...
	function addMemberAdmin($admin) {
		$query = "	INSERT INTO dlp_group_admins
	...
	function removeMemberAdmin($admin) {
		$query = "	DELETE FROM  dlp_group_admins
					WHERE
	...
	function getMemberAdmins($group) {
		...
		$query = "	SELECT *
					FROM dlp_group_admins
					JOIN ".$this->database."galette_adherents ON id_adh = gad_member_id
					WHERE
						gad_group_id = :gad_group_id \n";
