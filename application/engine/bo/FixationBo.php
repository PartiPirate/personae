...
class FixationBo {
	...
	function __construct($pdo, $database = null) {
		if ($database) {
	...
	static function newInstance($pdo, $database = null) {
		return new FixationBo($pdo, $database);
	}

	function create(&$fixation) {
		$query = "	INSERT INTO dlp_fixations () VALUES ()	";
	...
	function update($fixation) {
		$query = "	UPDATE dlp_fixations SET ";
...
//		echo showQuery($query, $fixation);
	...
	function getFixations($filters = null) {
		...
		$query .= "	LEFT JOIN dlp_themes ON fix_theme_id = the_id AND fix_theme_type = 'dlp_themes' AND the_deleted = 0 \n ";
...
//		}
		...
		if ($filters && isset($filters["with_fixation_members"]) && $filters["with_fixation_members"]) {
			$query .= "	LEFT JOIN dlp_fixation_members ON fix_id = fme_fixation_id \n";
			$query .= "	LEFT JOIN ".$this->database."galette_adherents ON fme_member_id = id_adh \n";
		}
...
//		echo showQuery($query, $args);
	...
	function addFixationMember($fixationMember) {
		$query = "	INSERT INTO dlp_fixation_members
	...
	function removeFixationMember($fixationMember) {
		...
		$query = "	DELETE FROM  dlp_fixation_members
					WHERE
